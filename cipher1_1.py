...
def CHash(string):
    ...
    for x in bigblocks:
        ...
        #next, we compute this complex function on each of the 8 blocks


        c = [collatz(ordstr_s(i)**16+ordstr_s2(i)**8) for i in blocks]

        #ordstr_a (appending) doesn't allow for chaotic differences between a bit change from 0 to 1
        #ordstr_s (adding) and then raising to a high power accentuates the differences to chaotic proportions
        #this function was designed in a way to accentuate even the most minute differences
...
#this first one was not used in prototypes but not the final model
def ordstr_a(string):
    out = ""
    for i in string:
        out += str(ord(i))
...
x = CHash(open("hashfile.txt").read())
